# IncidentPilot V14.10 使用与实现指南

[返回项目首页](README.md) · [版本记录](CHANGELOG.md)

本指南面向需要安装、运行、复现实验或深入理解实现的读者。所有终端命令均在项目根目录执行。项目价值、核心设计和精简结果请先阅读 README。

## 阅读导航

- [安装与 API 配置](#1-快速开始)
- [CLI 演示与 Markdown 报告](#15-运行产品化演示)
- [持久化调查与审批恢复](#16-v13-稳定持久化命令)
- [Harness 与补丁验证](#17-第一次测试-safe-test-harness)
- [架构与数据流](#4-从输入到报告的完整数据流)
- [RAG 检索实现](#7-本地-rag-原理)
- [假设与证据链](#8-hypothesis--and-evidence-driven-diagnosis)
- [确定性 Evaluation 与 LLM Judge](#10-确定性-evaluation)
- [实验命令和数据集分组](#11-工具消融runtime-evidence成本与稳定性实验)
- [离线测试](#12-测试)
- [常见问题](#15-常见问题)
- [当前限制](#16-当前限制)
...
- `README.md`：面向项目评审和面试，保留当前能力、架构、代表性案例、可核对结果与边界；
- `GUIDE.md`：维护安装配置、命令、实现原理、评测复现、故障排查与详细限制；
- `CHANGELOG.md`：新增该版本的“新增、修改、修复、安全、验证和兼容性”记录。

版本演进写入 CHANGELOG；操作教程写入 GUIDE，不再向 README 追加调试日志或命令手册。下方附录仅保留本次文档迁移前的验收背景，后续迭代不再追加。
...
版本演进单独记录在 [CHANGELOG.md](CHANGELOG.md)。本节保留原 README 的阶段性验收记录；历史结果不代表当前版本的每次运行表现。
