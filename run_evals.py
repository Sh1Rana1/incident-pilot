...
def format_summary(result: ExperimentRun) -> str:
    rows = [
        "逐次结果",
        "Profile     案例                     次数  通过  根因  代码  文档  引用  落地 Claim 违规 假设 早停 利用 压缩  模型  工具 Token",
        "-" * 151,
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行 IncidentPilot V8.1.1 证据保全与稳定性实验（会调用真实模型）"
    )
    parser.add_argument(
        "--case",
        action="append",
...
def main() -> None:
    ...
    profiles = list(TOOL_PROFILES) if arguments.compare else (arguments.profiles or ["full"])
