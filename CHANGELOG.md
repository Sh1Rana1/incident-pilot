...
本文件记录版本演进、行为变化和迁移注意事项。项目概览与评审入口见 [README.md](README.md)，安装、使用、架构与完整原理见 [GUIDE.md](GUIDE.md)。

## 文档维护约定

以后每次发布或完成一个版本，按变更范围同步维护：

1. `README.md`：面向面试官和项目评审，描述当前能力、核心设计、代表性结果及边界；
2. `GUIDE.md`：维护完整操作教程、配置、实现原理、评测复现与故障排查；
3. `CHANGELOG.md`：记录新增、修改、修复、验证结果和兼容性影响。

如果只是修正文案且不改变程序行为，可以只更新相关文档；代码、配置、命令、数据结构或安全边界变化应同步更新相关说明与版本记录。
...
- 将原 README 的安装、配置、命令教程、实现原理、面试讲解及详细限制迁移到根目录 `GUIDE.md`，提供章节导航；原阶段性验收背景保留为指南附录。
- README 改为面向面试官和项目评审的项目首页，包含代表性上游根因案例、调查流程图、核心机制、代码阅读入口和适用边界。
- 分开呈现 V13/V14.3 同口径历史对照与 V14.9 严格正式结果，明确 development 重叠、challenge 排除、单次样本和同模型 Judge 限制，以及完整原始报告未随仓库发布。
- 此次仅调整文档组织，不修改 Agent、工具权限、评分规则或已冻结实验数据。
- 迁移后 216 项离线测试全部通过，本地文档链接检查通过；四个 Tool Profile 均继续拒绝通过文件读取、搜索和目录列举暴露指南，现有文档取证隔离保持有效。
...
- 将必需代码文件覆盖门槛从 50% 提升到 100%；多文件调用链必须全部取得并引用真实 Evidence，不能只找到直接报错点就通过。
...
- 正式审计在干净提交 `af1be13` 上完成：166/166 项检查通过、失败 0 项；24/24 次模块/脚本故障复现与 22/22 个 Harness Check 行为符合预期，关键词来源缺口、RAG 漏检和受保护资产泄漏均为 0。V13 两份及 V14.3 一份冻结原始报告的 SHA-256 全部匹配；审计专项 70 项、完整离线 198 项和 `doctor` 均通过。
- 审计报告固化为 `demo_app/evals/results/v14.7-deterministic-audit.json`，SHA-256 为 `ee970f210ab7c802eafec2602266e382adf82881ac6d6bc124a510f16e4dd890`，报告元数据记录 `working_tree_dirty=false`。本阶段没有运行 DeepSeek，也没有产生新的模型通过率或 Token 数据。
